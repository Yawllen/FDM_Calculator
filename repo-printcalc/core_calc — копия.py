...
def compute_volume_cm3(V_mm: np.ndarray, T: np.ndarray, *, mode: str, meta: dict) -> float:
    ...
    Поддерживаемые режимы:
      • tetra (alias: fast) — тетраэдры по треугольникам (точный объём для замкнутого меша)
      • bbox — ограничивающий параллелепипед по габаритам

    Режим stream (потоковый STL) намеренно удалён: в текущем пайплайне для FDM-модели
    всё равно требуется меш, а двойное чтение файла ухудшает UX/детерминизм.
    """
