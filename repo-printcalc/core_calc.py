...
def compute_volume_cm3(V_mm: np.ndarray, T: np.ndarray, *, mode: str, meta: dict) -> float:
    """Возвращает объём модели в см^3.

   Поддерживаемые режимы:
  • tetra — тетраэдры по треугольникам (точный объём для замкнутого меша)
    (legacy alias: fast — для обратной совместимости)
  • bbox — ограничивающий параллелепипед по габаритам

    Режим stream (потоковый STL) удалён: в текущем пайплайне для FDM-модели всё равно требуется меш,
    а отдельный путь чтения STL ухудшает UX/детерминизм.
    """
    mode_norm = (mode or "").strip().lower()
    if mode_norm in {"tetra", "fast"}:
        meta = meta or {}
...
def parse_stl(path: str):
    ...
    verts, tris, idx_map = [], [], {}
    def _vertex_index(xyz, tri_idx: int, vi: int) -> int:
    ...
    try:
        with open(path, 'rb') as f:
            ...
            for tri_idx in range(count):
                ...
                face = (
                    _vertex_index(v0, tri_idx, 0),
                    _vertex_index(v1, tri_idx, 1),
                    _vertex_index(v2, tri_idx, 2),
                )
                tris.append(face)
                f.read(2)
    except struct.error as e:
